...
export type PathPredicate = (absPath: string, isDir: boolean) => boolean; // true => skip

export async function isDir(uri: vscode.Uri): Promise<boolean> {
  try {
    const s = await vscode.workspace.fs.stat(uri);
    return !!(s.type & vscode.FileType.Directory);
  } catch {
    return false;
  }
}

export async function expandDirAllFiles(
  dirFs: string,
  maxDepth: number,
  out: string[],
  depth = 0
  ...
  let entries: string[] = [];
  try {
    entries = await fs.readdir(dirFs);
  } catch {
    return;
  }
  ...
  for (const name of entries) {
    ...
    if (st.isDirectory()) {
      await expandDirAllFiles(full, maxDepth, out, depth + 1);
    } else if (st.isFile()) {
      out.push(full);
    }
  }
...
export async function buildTree(
  ...
  function walk(rel: string, prefix: string, depth: number) {
    ...
    try {
      dirents = fsSync.readdirSync(abs, { withFileTypes: true });
    } catch {
      return;
    }
    dirents = dirents
      .filter(
        (d) =>
          !(shouldSkip && shouldSkip(path.join(abs, d.name), d.isDirectory()))
      )
