      ...
      limit(async () => {
        try {
          ...
          if (isOversized && !cfg.splitOversizedFiles && !ignoreSizeLimit) {
            skipped.push({
              path: u.fsPath,
              reason: `size>${cfg.maxBytesPerFile}`,
            });
            return;
          }

          const raw = await vscode.workspace.fs.readFile(u);
          if (!ignoreSizeLimit && isOversized && !cfg.splitOversizedFiles) {
            skipped.push({
              path: u.fsPath,
              reason: `size>${cfg.maxBytesPerFile}`,
            });
          ...
          // detect binary early (on header slice)
          const head = raw.subarray(0, Math.min(raw.length, 8192));
          if (cfg.detectBinary && isProbablyBinary(head)) {
            skipped.push({ path: u.fsPath, reason: "binary" });
            return;
          }
          ...
          if (isOversized && cfg.splitOversizedFiles) {
            ...
            chunks.forEach((chunk, i) => {
              ...
              blocks.push(
                `=== FILE PART: ${rel} | CHUNK ${
                  i + 1
                }/${total} | LINES:${lines} BYTES:${bytes} | LANG:${
                  lang ?? "plain"
                } | HASH:sha256:${hash} ===
\`\`\`${lang ?? ""}
${chunk}
